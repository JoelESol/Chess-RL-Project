...
def train(net, dataset, epoch_start = 0, epoch_stop = 20, cpu=0):
    ...
    trainloader = DataLoader(train_set, batch_size=30, shuffle=True, num_workers=0, pin_memory=False)
    losses_per_epoch = []
    for epoch in range(epoch_start, epoch_stop):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if cuda:
                state, policy, value = state.cuda().float(), policy.cuda().float(), value.cuda().float()
            optimizer.zero_grad()
